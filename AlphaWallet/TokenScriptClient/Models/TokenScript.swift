...
extension TokenScript {
    ...
    static let oldNoLongerSupportedTokenScriptNamespaceVersions = ["2019/05"].map { "\(tokenScriptNamespacePrefix)\($0)/tokenscript" } + ["2019/04"].map { "\(tokenScriptNamespacePrefix)\($0)/tokenscript" }
